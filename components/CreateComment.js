...
const CreateComment = ({postId, afterAddingComment}) => {
    ...
    const handleSubmit = (e) => {
        ...
        axios
        .post(FACEBOOK_CLONE_ENDPOINT + "/" + postId + "/comments/" + userId , body, {
    ...
    return <div className="w-full px-2">
        <form className="w-full">
            <textarea type="textarea"
                ...
                className="w-full rounded-lg h-12 focus:outline-none bg-gray-100 px-3 py-2"
            ></textarea>
        </form>
            <div className="flex justify-evenly py-2">
            <button className="rounded bg-teal-400 hover:bg-teal-500 text-white px-6 py-1 w-full" onClick={handleSubmit}>Comment</button>
        </div>
    </div>;
};
